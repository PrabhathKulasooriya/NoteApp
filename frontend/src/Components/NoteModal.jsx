...
import { X, Tag, Users, Type, AlignLeft } from "lucide-react";
...
const NoteModal = ({ setModalOpen, existingNote = null }) => {
    ...
    try {
      ...
      if (existingNote) {
        ...
        response = await axios.post(
          `/api/notes/update/${existingNote._id}`,
          noteData,
          {
            headers: { token: token },
          },
        ...
        response = await axios.post("/api/notes/save", noteData, {
          headers: { token: token },
        });
      }
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-slate-800">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6 custom-scrollbar max-h-[70vh]">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-white dark:bg-slate-800/50 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
              <ReactQuill
                theme="snow"
                value={content}
                onChange={setContent} 
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="Comma separated emails (e.g. friend@email.com, boss@email.com)"
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/30 border-t border-gray-100 dark:border-slate-800 flex items-center justify-end gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={!title || !content || !token || content === "<p><br></p>"}
